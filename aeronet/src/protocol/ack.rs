...
/// * https://gafferongames.com/post/reliability_ordering_and_congestion_avoidance_over_udp/, *Reliable Acks*
...
pub struct Writer<const CAP: usize> {
    seq_buf: [Seq; CAP],
    packets: [Option<PacketState>; CAP],
}
...
impl<const CAP: usize> Writer<CAP> {
    ...
    pub fn get(&self, seq: Seq) -> Option<PacketState> {
    ...
    pub fn insert(&mut self, seq: Seq, data: PacketState) {
        let index = usize::from(seq.0) % CAP;
