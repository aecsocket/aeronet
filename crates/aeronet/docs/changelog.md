...
- Made `Server` only be added to opened servers, and have this component store the `opened_at: Instant`
- Removed `Opened`
...
- Added `aeronet_webtransport/self-signed` feature, mapping to `wtransport/self-signed` (enabled by default)
- [#15](https://github.com/aecsocket/aeronet/issues/15): fixed a bug in `aeronet_transport` where receiving a previously-received fragment (in a resent packet) would lead to an error death spiral
...
- Removed the concept of protocols
  - `u8`s and lanes are baked into the core `-Transport` traits, simplifying working with networked transports
