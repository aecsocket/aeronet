...
/// Provides a platform-agnostic way to spawn futures for driving the Iroh IO
/// layer.
///
/// Using Iroh endpoints requires spawning tasks on an async runtime. However,
/// which runtime to use exactly, and how that runtime is provided, is
/// target-dependent. This resource exists to provide a platform-agnostic way
/// of spawning these tasks.
///
/// # Platforms
///
/// ## Native
///
/// On a native target, this holds a handle to a `tokio` runtime, because Iroh
/// currently uses this async runtime.
///
...
/// `wasm-bindgen`.
///
/// Use the [`Default`] impl to create a new [`IrohRuntime`] on WASM.
...
#[cfg(not(target_family = "wasm"))]
impl From<tokio::runtime::Handle> for IrohRuntime {
...
impl IrohRuntime {
    ...
    ///
    /// If you are already in a task context, use [`IrohRuntime::spawn`] to
    /// avoid having to pass around [`IrohRuntime`].
