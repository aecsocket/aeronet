...
///
/// Since you can open multiple streams of the same type on a connection, we
/// need a way to index which exact stream we're using when sending data. This
/// struct provides an opaque way to represent this.
...
impl StreamId {
    ...
    ///
    /// Sending data using a stream ID created from this function may lead to
    /// bugs or panics, as an invalid stream will be used to send data.
    /// Prefer the functions on [`TransportStreams`] instead to obtain a
    /// stream for your current side.
...
/// The simplest type of "stream" - a [`TransportStream::Datagram`] is useful
/// for cases when it is OK if a few messages are lost or sent in the wrong
/// order, as they can be sent with little overhead, such as a player's position
/// update. A proper stream like [`TransportStream::Bi`] can be used for cases
/// in which sending data should be ordered and reliable, at the cost of the
/// message potentially being received later, such as a chat message or an
/// interaction event. Multiple streams of the same type can be opened to avoid
/// head-of-line blocking, where a stream is stuck waiting for a message that
/// was sent ages ago, disallowing any new messages to be received until that
/// one is received.
///
...
pub enum TransportStream {
    ...
    Datagram,
    /// A bidirectional stream, supporting both client-to-server and
...
/// Defines which streams will be created when a client/server connection is
/// established.
///
/// When a connection is started, the transport opens all required channels
/// during the establish step. Channels cannot be opened or closed afterwards.
/// This struct defines which channels will be created during this step.
///
/// # Usage
///
/// ```
